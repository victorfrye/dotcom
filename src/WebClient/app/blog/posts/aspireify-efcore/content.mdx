...
When Aspire launched, the answer still was a lot of manual work. You could write a [custom worker to programmatically execute migrations](https://aspire.dev/integrations/databases/efcore/migrations/) and wait for it's completion before starting your app. However, adding new migrations was limited as you need to provide names for each and or manage the execution of `dotnet ef` commands yourself. This was a huge improvement over the manual way, but required a lot of custom code and management.
